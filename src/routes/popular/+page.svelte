<script>
	...
	import PopularPageAd from '$lib/components/PopularPageAd.svelte';
	...
	async function fetchSneakers() {
		try {
			const res = await fetch('/api/popular?limit=40');
			const data = await res.json();
			results = data.filter(
				(item) =>
					!item.shoeName.includes('Sunglasses') &&
					!item.shoeName.includes('Jersey') &&
					!item.shoeName.includes('Card') &&
					!item.shoeName.includes('Backpack') &&
					!item.shoeName.includes('Bag')
			);
			results.splice(1, 0, 'ad');
		} catch (e) {
			error = e.message;
		} finally {
			loading = false;
...
<svelte:head>
	...
	<meta
		name="description"
		content="Discover the most popular and trending sneakers on SNKRFinder. Find hot deals on Air Jordans, Air Force 1s, and other trending sneakers with the best resale prices."
	/>
	<meta
		name="keywords"
		content="popular sneakers, trending sneakers, hot sneakers, best selling sneakers, popular Jordan deals, trending Nike deals, hot sneaker releases, popular sneaker deals"
	/>
	...
	<meta
		...
		content="Discover the most popular and trending sneakers on SNKRFinder. Find hot deals on Air Jordans, Air Force 1s, and other trending sneakers."
	/>
		...
		<div class="results">
			...
			{:else}
				{#each results as item}
					{#if item === 'ad'}
						<div class="grid-item ad-item"><PopularPageAd /></div>
					{:else}
